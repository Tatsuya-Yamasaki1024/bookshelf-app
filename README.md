...
書籍の登録・更新・削除APIには、Laravel SanctumによるBearer Token認証を導入しています。

認証が必要なAPIでは、リクエストヘッダーに以下の形式でSanctumトークンを設定します。

```http
Authorization: Bearer {Sanctumトークン}
```
...
| HTTPメソッド | URI                    | 認証 |
| ------------ | ---------------------- | ---- |
| POST         | `/api/v1/books`        | 必須 |
| PUT          | `/api/v1/books/{book}` | 必須 |
| DELETE       | `/api/v1/books/{book}` | 必須 |

GET系エンドポイントは認証不要で利用できます。

PUT・DELETEは、Sanctumによる認証に加えて、対象書籍の所有者のみ実行できます。

未認証の場合は `401 Unauthorized`、認証済みでも対象書籍を操作する権限がない場合は `403 Forbidden` を返します。
